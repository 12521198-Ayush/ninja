            ...
            <div class="row">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12 mb-3">
                                <div class="d-flex align-items-center gap-3">
                                    <div class="wallet-display d-flex align-items-center gap-2">
                                        <span class="analytics-icon">
                                            <i class="las la-wallet" style="font-size:22px"></i>
                                        </span>
                                        <div>
                                            <div class="h6 mb-0">{{ __('wallet_balance') }} 
                                                <span id="wallet_balance_error" class="text-danger d-none"><i class="las la-exclamation-circle"></i></span>
                                            </div>
                                            <div id="wallet_balance" class="fw-bold" data-initial="{{ number_format($wallet_balance ?? 0, 2) }}">
                                                {{ number_format($wallet_balance ?? 0, 2) }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="col-xxl-9 col-xl-8 col-lg-8 col-md-8">
                    <div class="row">
                        ...
                        <div class="col-xxl-4 col-xl-4 col-md-6 col-sm-6">
                            <a href="{{ route('client.whatsapp.campaigns.index') }}">
                                <div class="bg-white redious-border mb-4 p-20 p-sm-30 analytics-box">
                                    <div class="analytics clr-2">
                                        ...
                                        <div class="analytics-content">
                                            <h4>{{ $usages['campaign'] }}/{{ $active_subscription->campaign_limit == -1 ? __('unlimited') : $active_subscription->campaign_limit }}
                                            </h4>
                                ...
                                <div class="analytics clr-4">
                                    ...
                                    <div class="analytics-content">
                                        <h4>
                                            ...
                                            /
                                            {{ $active_subscription->conversation_limit == -1 ? __('unlimited') : ReadableNumbers::make($active_subscription->conversation_limit) }}
                                        </h4>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="walletHistoryModalLabel">{{ __('wallet_history') }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div id="wallet_history_spinner" class="text-center py-3 d-none">
                        <div class="spinner-border" role="status">
                            <span class="visually-hidden">{{ __('loading') }}</span>
                        </div>
                    </div>
                    <div id="wallet_history_error" class="text-center text-danger py-3 d-none"></div>
                    <div id="wallet_history_loading" class="text-center py-3 d-none">{{ __('loading') }}...</div>
                    <div class="table-responsive d-none" id="wallet_history_table_wrap">
                        <table class="table table-striped table-sm" id="wallet_history_table">
                            <thead>
                                <tr>
                                    <th>{{ __('date') }}</th>
                                    <th>{{ __('type') }}</th>
                    ...
                    <div id="wallet_history_empty" class="text-center d-none py-3">{{ __('no_history_found') }}</div>
                </div>
...
@push('js')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            // small helper that prefers axios but falls back to fetch
            ...
            // balance is preloaded server-side; optional client-side override not required
            var CLIENT_BALANCE = @json($wallet_balance ?? 0);

            // Client id available to JS for requests
            var CLIENT_ID = @json(Auth::id());
            ...
            async function updateWalletBalance() {
                ...
                const errorIcon = document.getElementById('wallet_balance_error');

                try {
                    errorIcon.classList.add('d-none');

                    // Call the provided API endpoint which returns { "balance": "100.00" }
                    const url = `/api/wallet/get-balance/${CLIENT_ID}`;
                    const response = await fetch(url, {
                        headers: {
                            'Accept': 'application/json'
                        },
                        // include credentials only if your API requires session auth
                    ...
                    // Accept multiple response shapes: { balance: '100.00' } or { data: { balance: ... } }
                    const balance = data?.balance ?? data?.data?.balance ?? data?.data ?? null;
                    ...
                    errorIcon.setAttribute('title', error.message || 'Failed to update balance');
                    // Restore initial value from server-side render
                    walletBalance.textContent = walletBalance.dataset.initial;
                }
            }
            
            // Update balance initially and every 30 seconds
            updateWalletBalance();
            setInterval(updateWalletBalance, 30000);

            // reusable history fetcher - sends client_id as query param
            function fetchWalletHistory(clientId) {
                var loading = document.getElementById('wallet_history_loading');
                var tableWrap = document.getElementById('wallet_history_table_wrap');
                var empty = document.getElementById('wallet_history_empty');
                var tbody = document.querySelector('#wallet_history_table tbody');

                var spinner = document.getElementById('wallet_history_spinner');
                var errorBox = document.getElementById('wallet_history_error');
                ...
                // Use the provided API endpoint: /api/{userId}/balance-history
                var url = '/api/' + encodeURIComponent(clientId) + '/balance-history';

                fetchJson(url)
                    .then(function (resData) {
                        // Expected shape: { user: {...}, balance_history: [ ... ] }
                        var data = resData;
                        var items = [];
                        if (data && Array.isArray(data.balance_history)) {
                            items = data.balance_history;
                        ...
                        items.forEach(function (it) {
                            var date = it.created_at || it.date || it.created || it.time || '';
                            var type = it.type || (it.amount && it.amount > 0 ? 'credit' : 'debit') || '';
                            var amount = it.amount !== undefined ? it.amount : (it.value !== undefined ? it.value : '0');
                            var balanceAfter = it.balance_after !== undefined ? it.balance_after : '';
                            var desc = it.note || it.description || it.details || '';
                            var tr = document.createElement('tr');
                            var tdDate = document.createElement('td'); tdDate.textContent = formatDateIST(date);
                            var tdType = document.createElement('td'); tdType.textContent = type;
                            var tdAmount = document.createElement('td'); tdAmount.textContent = (Number(amount) || 0).toFixed(2);
                            var tdBalance = document.createElement('td'); tdBalance.textContent = balanceAfter ? Number(balanceAfter).toFixed(2) : '';
                            var tdDesc = document.createElement('td'); tdDesc.textContent = desc;
                            tr.appendChild(tdDate); tr.appendChild(tdType); tr.appendChild(tdAmount); tr.appendChild(tdBalance); tr.appendChild(tdDesc);
                    ...
                    .catch(function (err) {
                        ...
                        if (tableWrap) tableWrap.classList.add('d-none');
                        if (errorBox) {
                            errorBox.textContent = '{{ __('wallet_history_load_error') }}';
            ...
            // open history when clicking on the wallet amount
            var walletAmountEl = document.getElementById('wallet_balance');
            if (walletAmountEl) {
                walletAmountEl.style.cursor = 'pointer';
                walletAmountEl.setAttribute('title', '{{ __('click_for_history') }}');
                walletAmountEl.addEventListener('click', function () {
                    // open bootstrap modal programmatically
                    var modalEl = document.getElementById('walletHistoryModal');
                    if (modalEl && typeof bootstrap !== 'undefined' && bootstrap.Modal) {
                        var modal = new bootstrap.Modal(modalEl);
                        modal.show();
                    } else {
                        // fallback: if there's a trigger element, try to click it
                        var btn = document.querySelector('[data-bs-target="#walletHistoryModal"]');
                        if (btn) btn.click();
                    }
                    // load history after opening
                    fetchWalletHistory(CLIENT_ID);
                });
            }
        });
    </script>
