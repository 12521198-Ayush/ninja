...
class WaCampaignRepository
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            // }
            if ($user->wallet_balance < 1) {
                return $this->formatResponse(false, __('Insufficient Balance'), 'client.whatsapp.campaigns.index', []);
            }
            
            $charges = 0;
            
            if($template->category === 'UTILITY'){
                $charges = $user->utility_price;
            }else if($template->category === 'AUTHENTICATION'){
                $charges = $user->auth_price;
            }else {
                $charges = $user->marketing_price;
            }

            $amount = $contacts->count()* $charges;
            
            $user->wallet_balance = round($user->wallet_balance - $amount, 4);
            $user->save();
