...
class WhatsappRepository
{
    ...
    protected $webhookForwardingService;
    ...
    public function __construct(
        ...
        WebhookForwardingService $webhookForwardingService
    ) {
        ...
        $this->webhookForwardingService = $webhookForwardingService;
    }
    ...
    private function handleIncomingMessage($value, $client)
    {
        try {
            ...
            $savedMessage = $this->saveIncommingMessage($contact, $content, $client, $is_contact_msg, $is_campaign_msg, $type, $message_id);

            // Forward to client's webhook if enabled (with saved message containing file paths)
            $this->forwardToClientWebhook($client, $value, $contact, $type, $savedMessage);
    ...
    /**
     * Forward incoming message to client's webhook URL
     */
    private function forwardToClientWebhook($client, $value, $contact, $type, $savedMessage = null)
    {
        try {
            $baseUrl = rtrim(config('app.url'), '/');
            
            $processedData = [
                'message_id' => $value['messages'][0]['id'] ?? null,
                'from' => $value['messages'][0]['from'] ?? null,
                'contact_name' => $value['contacts'][0]['profile']['name'] ?? 'Unknown',
                'contact_id' => $contact->id,
                'type' => $type,
                'timestamp' => $value['messages'][0]['timestamp'] ?? time(),
                'metadata' => [
            ...
            // Add message content based on type with public URLs for media
            if ($type == 'text') {
                $processedData['text'] = $value['messages'][0]['text']['body'] ?? '';
            } elseif ($type == 'image') {
                $processedData['image'] = $value['messages'][0]['image'] ?? [];
                if ($savedMessage && $savedMessage->header_image) {
                    $processedData['image']['public_url'] = $baseUrl . '/storage/' . ltrim($savedMessage->header_image, '/');
                }
            } elseif ($type == 'audio') {
                $processedData['audio'] = $value['messages'][0]['audio'] ?? [];
                if ($savedMessage && $savedMessage->header_audio) {
                    $processedData['audio']['public_url'] = $baseUrl . '/storage/' . ltrim($savedMessage->header_audio, '/');
                }
            } elseif ($type == 'video') {
                $processedData['video'] = $value['messages'][0]['video'] ?? [];
                if ($savedMessage && $savedMessage->header_video) {
                    $processedData['video']['public_url'] = $baseUrl . '/storage/' . ltrim($savedMessage->header_video, '/');
                }
            } elseif ($type == 'document') {
                $processedData['document'] = $value['messages'][0]['document'] ?? [];
                if ($savedMessage && $savedMessage->header_document) {
                    $processedData['document']['public_url'] = $baseUrl . '/storage/' . ltrim($savedMessage->header_document, '/');
                }
            } elseif ($type == 'location') {
                $processedData['location'] = $value['messages'][0]['location'] ?? [];
                if ($savedMessage && $savedMessage->header_location) {
                    $processedData['location']['google_maps_url'] = $savedMessage->header_location;
                }
            } elseif ($type == 'contacts' || $type == 'contact') {
                $processedData['contacts'] = $value['messages'][0]['contacts'] ?? [];
            } elseif ($type == 'button') {
                $processedData['button'] = $value['messages'][0]['button'] ?? [];
            } elseif ($type == 'interactive') {
                $processedData['interactive'] = $value['messages'][0]['interactive'] ?? [];
            }

            $this->webhookForwardingService->forwardIncomingMessage($client, $value, $processedData);
        } catch (\Exception $e) {
            Log::error('Error forwarding to client webhook', ['error' => $e->getMessage()]);
        }
    }
