...
class MessageController extends Controller
{
    ...
    public function sendTemplate(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $priceInfo = $this->resolveTemplatePricing($user, $template);
            $charge    = $priceInfo['amount'];
            ...
            if ($charge > 0) {
                ...
                if ($lockedUser->wallet_balance < $charge) {
                    DB::rollBack();
                    return $this->responseWithError(__('sorry_balance_not_sufficient'), [], 200);
                }

                $lockedUser->wallet_balance = round($lockedUser->wallet_balance - $charge, 4);
                $lockedUser->save();
    ...
    private function resolveTemplatePricing(User $user, Template $template): array
    {
        $category = strtoupper((string) $template->category);

        return match ($category) {
            'MARKETING'      => [
                'amount' => (float) ($user->marketing_price ?? 0),
                'note'   => 'Template message charge - Marketing',
            ],
            'UTILITY'        => [
                'amount' => (float) ($user->utility_price ?? 0),
                'note'   => 'Template message charge - Utility',
            ],
            'AUTHENTICATION' => [
                'amount' => (float) ($user->auth_price ?? 0),
                'note'   => 'Template message charge - Authentication',
            ],
            default          => [
                'amount' => (float) ($user->marketing_price ?? 0),
                'note'   => 'Template message charge - '.($category ? ucwords(strtolower($category)) : 'Marketing'),
            ],
        };
    }
